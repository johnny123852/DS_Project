...
def getedge(txin_df, txout_df):
    ...
    edge_havena = spark.sql("SELECT A1.address AS src, A2.address as dst, 1 as relationship, A1.txid  \
                 from txintable as A1, txouttable as A2 \
                 where A1.txid = A2.txid")
    #edge.collect()
    edge = edge_havena.dropna()  #剩176273863筆
    print(edge.take(1))
    print(edge.count())
    return edge
...
def getPageRank(g):
    # Run PageRank algorithm, 改以自動收斂參數計算
